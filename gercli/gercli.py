...
def main():
    ...
    thread_parser = subparsers.add_parser('threads', parents=[parent_parser],
            help='Print comment threads', aliases = 't')
    ...
    thread_parser.add_argument('--count', action='store_true',
            help='Print the number of threads')
    thread_parser.set_defaults(func=thread.main,
    ...
    change_parser.add_argument('--count', action='store_true',
            help='Print the number of changes')
    change_parser.set_defaults(func=change.main,
            required_args=['user', 'password', 'server'])
    set_defaults_from_config_file(change_parser, 'changes')

    args = parser.parse_args()

    subparser = {'threads': thread_parser, 't': thread_parser,
            'changes': change_parser, 'c': change_parser}[args.subparser]
